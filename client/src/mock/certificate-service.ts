...
export const createMockCertificateRequest = () => ({
  ...
  type: faker.helpers.arrayElement(['BARANGAY_CLEARANCE', 'RESIDENCY', 'INDIGENCY', 'PERMIT']),
  status: faker.helpers.arrayElement(['PENDING', 'APPROVED', 'DENIED', 'READY_FOR_PICKUP']),
  purpose: faker.lorem.sentence(),
  requestedAt: faker.date.recent().toISOString(),
  processedAt: faker.date.recent().toISOString(),
});

const mockCertificates = Array.from({ length: 50 }, createMockCertificateRequest);
