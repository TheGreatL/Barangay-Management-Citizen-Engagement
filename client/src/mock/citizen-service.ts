...
export const createMockCitizenDocument = () => ({
  ...
  documentType: faker.helpers.arrayElement(['BARANGAY_CLEARANCE', 'RESIDENCY', 'INDIGENCY', 'PERMIT']),
  status: faker.helpers.arrayElement(['pending', 'processing', 'ready_for_pickup', 'completed', 'rejected']),
  purpose: faker.lorem.sentence(),
  createdAt: faker.date.recent().toISOString(),
  pickupDate: faker.date.future().toISOString(),
});

const mockCitizenDocuments = Array.from({ length: 20 }, createMockCitizenDocument);
...
export const createMockCitizenComplaint = () => ({
  ...
  status: faker.helpers.arrayElement(['pending', 'investigating', 'resolved', 'closed']),
  category: faker.helpers.arrayElement(['noise', 'garbage', 'conflict', 'infrastructure']),
  createdAt: faker.date.recent().toISOString(),
  updatedAt: faker.date.recent().toISOString(),
});

const mockCitizenComplaints = Array.from({ length: 15 }, createMockCitizenComplaint);
