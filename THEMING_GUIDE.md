...
All colors are defined as CSS variables in `client/src/shared/styles/global.css`. To change the theme, simply update the
color values in the `:root` selector.
...
The application automatically detects the system's dark mode preference. To force dark mode, add the `dark` class to the
`html` element:
...
```tsx
<div className='border border-slate-200 bg-white text-slate-900 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-50'>
  Themed Component
</div>
```
